...
- Added WARPi-side Mission Control visibility through `warpi mission status` and bounded `warpi mission check-in`. The commands report configured endpoints, reachability, health, authentication presence, stable device identity, last check-in metadata, and send a bounded heartbeat payload when the server is reachable. Mission Control failure remains non-blocking for WARPi.
- Documented the 2026-08-13 Mission Control access blocker: `mission01` is Tailscale-reachable at `100.71.99.18`, but expected API ports refused connections, SSH/Tailscale SSH from OpenClaw was not authorized, and the routine Proxmox token remains denied for protected VM `110`.
- Completed the Mission Control server-side repair after approved `kernel-agent` access to `mission01` was restored. Root cause for TCP/8000 refusal was `mission-api` crash-looping while resolving/connecting to the `postgres` service; PostgreSQL and Redis were healthy. Recreated/rebuilt only the API container, preserved persistent volumes, restored `/health`, added bearer-authenticated WARPi check-in/device endpoints, initialized non-destructive device/heartbeat tables, installed root-owned WARPi client token storage, verified offline/reconnection behavior, and kept WARPi NORMAL with live executor disabled.
- Fixed `warpi mission status` and `warpi mission check-in` presentation for protected token/check-in metadata access and the human-readable mode/state summary.
- Added Mission Control hardening/readiness updates before Milestone E: created the private `dustinvaughn91/WARPi-Mission-Control` source repository, tracked safe source/config/docs/scripts while excluding secrets and runtime state, enabled `onboot=1` for protected VM `110`, added the `mission01-weekly` Proxmox backup job without running an immediate backup, added and validated a custom-format PostgreSQL backup helper, and documented restore-only recovery procedure. External Wi-Fi remains not ready because no external Wi-Fi USB device enumerates on WARPi.
- Added Milestone E execution architecture while keeping live mutation disabled: adapter inventory, hard live-execution gate, default-unarmed arming framework, external Wi-Fi prerequisite enforcement, management-path watchdog, boot/interruption recovery status, fixture test command, richer Mission Control telemetry, and clearer `--apply` refusal reasons. WARPi remained NORMAL/CONNECTED and live executor stayed disabled.
- Added Milestone F controlled live-readiness validation after the Panda assessment radio enumerated. WARPi now records the approved Panda identity (`0e8d:7612`, `mt76x2u`, `phy1`, `wlan1`, `9c:ef:d5:f8:98:83`) as distinct from management `wlan0`/`phy0`, adds a status-only `warpi-transition-recovery.service` artifact, strengthens recovery recording and stale-arm clearing, updates the live gate so hardware validation no longer implies authorization, adds Milestone F fixtures, and records the end-of-day closeout report. Live FIELD execution remains disabled and unarmed. First live-test readiness is NO-GO until NANO/ASIX sidecar stability is restored because final closeout observed ASIX re-enumeration and loss of NANO IPv4/UI reachability.
