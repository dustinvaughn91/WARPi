...
The API exposes unauthenticated `/health` for dependency status and authenticated WARPi check-in/device endpoints. Bearer authentication uses a server-side secret mounted from `/srv/mission-control/secrets/warpi_api_token` and a WARPi client token at `/etc/warpi/mission-control.token`; secret values must never be printed or committed. PostgreSQL state persists under `/srv/mission-control/postgres`, Redis append-only data under `/srv/mission-control/redis`, and secrets under `/srv/mission-control/secrets`. The existing `/srv/mission-control` deployment is not currently a Git repository, so server source/config source-of-truth and application-aware backup policy remain open operational gaps.
