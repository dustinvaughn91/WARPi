...
Milestone F adds a deployable status-only boot recovery component, `warpi-transition-recovery.service`. The service runs `/usr/local/bin/warpi mode recovery-status --record`, writes a recovery recommendation under `/run/warpi`, clears stale executor arm state, and exits without changing networking, routes, firewall, services, or WARPi mode while live execution is disabled. It is intentionally narrow: classify state, fail closed on ambiguity, and never resume forward toward FIELD after interruption.

The approved external assessment radio is the Panda MT7612U adapter with USB `0e8d:7612`, driver `mt76x2u`, MAC `9c:ef:d5:f8:98:83`, observed as `phy1`/`wlan1`. It is distinct from the management radio `wlan0`/`phy0` on `CLS Health` and from the NANO sidecar `eth1`. Hardware validation updates FIELD readiness, but it does not authorize `--apply`; the live executor remains disabled and requires a future explicit operator-approved arm.
