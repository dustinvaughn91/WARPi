...
Mission Control is intended to observe WARPi health, telemetry, mode safety state, and future mission coordination over Tailscale. Current WARPi configuration probes `mission01` health endpoints such as `http://mission01:8000/health` and `http://100.71.99.18:8000/health`. The WARPi-side check-in payload includes device ID, timestamp, hostname, mode/state, Tailscale state, active interface/profile, transition state, last preflight/rollback readiness, executor backend/live-enabled state, sidecar identity, GPS state, and critical service status. Authentication is supported through a bearer-token file reference, but secret values must never be printed or committed.

As of the Mission Control integration pass on 2026-08-13, `mission01` was Tailscale-reachable at `100.71.99.18`, but expected API ports including `8000` refused connections from both OpenClaw and WARPi. SSH and Tailscale SSH access from OpenClaw were not authorized, and the routine Proxmox token is intentionally denied `VM.Audit` for protected VM `110`, so server-side repair, container inspection, and persistence verification require an approved `mission01` access path. WARPi-side Mission Control failure behavior is bounded and local operation continues.
