...
- Build a staged mutating executor simulation behind an explicit hard safety gate before any real mode apply path is approved.
- Implement the reversible mode-transition design in staged milestones; Milestone A transition-status, Milestone B preflight rollback-snapshot capture, and Milestone C rollback planning are complete, while live apply remains gated.
